...
def choose_thumbnail_src(
    ...
    # Check D:\Miru_Assets\<SET>\base\<CODE>.<ext>
...
def load_catalog_card_index() -> dict[str, dict]:
...
def _library_code_sort_key(code: str) -> tuple:
    ...
    if not m:
        return (text, 9999, 9999, "")
    prefix, set_num, card_num, suffix = m.groups()
    return (prefix, int(set_num), int(card_num), suffix or "")
            ...
            deck_leader_pool.append(
                {
                    ...
                    "colors": [t.strip() for t in str(card.get("color") or "").replace("|", "/").split("/") if t.strip()],
                    "power": str(card.get("power") or "").strip(),
                    "attribute": str(card.get("attribute") or "").strip(),
                    "effect_text": str(card.get("effect_text") or "").strip(),
                    "trigger_text": str(card.get("trigger_text") or "").strip(),
                    "trait_pills": [t.strip() for t in str(card.get("traits") or "").replace("|", "/").split("/") if t.strip()],
                    "color_tags": [t.strip() for t in str(card.get("color") or "").replace("|", "/").split("/") if t.strip()],
                }
            )
        else:
...
def api_card_variants(card_code: str):
    ...
    try:
        ...
        WHERE c.canonical_code = ?
          ...
          AND (
            TRIM(COALESCE(cv.image_path, '')) != ''
            OR TRIM(COALESCE(cv.image_url, '')) != ''
          )
        ORDER BY
        ...
        for row in rows:
            ...
            img_url_path = ""
            if image_path:
                full_filesystem_path = MIRU_ASSETS / image_path
                if full_filesystem_path.is_file():
                    img_url_path = "/img/" + image_path.replace("\\", "/")
            if not img_url_path and image_url:
                img_url_path = image_url
            if not img_url_path:
                continue
        ...
        base_first = [v for v in variants if v.get("variant_key") == "base" and v.get("is_base")]
        rest = [v for v in variants if not (v.get("variant_key") == "base" and v.get("is_base"))]
