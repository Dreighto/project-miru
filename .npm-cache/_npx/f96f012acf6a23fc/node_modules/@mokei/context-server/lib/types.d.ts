...
import type { CallToolResult, ClientMessage, CompleteRequest, CompleteResult, CreateMessageRequest, CreateMessageResult, ElicitRequest, ElicitResult, GetPromptResult, InitializeRequest, ListResourcesRequest, ListResourcesResult, ListResourceTemplatesRequest, ListResourceTemplatesResult, ListRootsRequest, ListRootsResult, LoggingLevel, ReadResourceRequest, ReadResourceResult, Resource, ResourceTemplate, ServerMessage, InputSchema as ToolInputSchema } from '@mokei/context-protocol';
...
export type CompleteHandler = (request: HandlerRequest<{
    params: CompleteRequest['params'];
}>) => CompleteResult | Promise<CompleteResult>;
export type PromptHandlerReturn = GetPromptResult | Promise<GetPromptResult>;
export type GenericPromptHandler = (request: HandlerRequest<{
    arguments: unknown;
}>) => PromptHandlerReturn;
export type TypedPromptHandler<Arguments> = (request: HandlerRequest<{
    arguments: Arguments;
}>) => PromptHandlerReturn;
...
export type ListResourcesHandler = (request: HandlerRequest<{
    params: ListResourcesRequest['params'];
}>) => ListResourcesResult | Promise<ListResourcesResult>;
export type ListResourceTemplatesHandler = (request: HandlerRequest<{
    params: ListResourceTemplatesRequest['params'];
}>) => ListResourceTemplatesResult | Promise<ListResourceTemplatesResult>;
export type ReadResourceHandler = (request: HandlerRequest<{
...
export type GenericToolHandler = (request: HandlerRequest<{
    arguments: Record<string, unknown>;
}>) => ToolHandlerReturn;
export type TypedToolHandler<Arguments> = (request: HandlerRequest<{
    arguments: Arguments;
}>) => ToolHandlerReturn;
...
export type ExtractToolTypes<T extends ToolDefinitions> = {
    [K in keyof T]: T[K] extends TypedToolDefinition<infer S> ? FromSchema<S> : T[K] extends GenericToolDefinition ? Record<string, unknown> : never;
};
/**
...
export type ExtractPromptTypes<T extends PromptDefinitions> = {
    [K in keyof T]: T[K] extends TypedPromptDefinition<infer S> ? FromSchema<S> : T[K] extends GenericPromptDefinition ? Record<string, unknown> : never;
};
/**
 ...
 * ```
 */
export type ExtractServerTypes<T extends {
    tools?: ToolDefinitions;
    prompts?: PromptDefinitions;
}> = {
    Tools: T['tools'] extends ToolDefinitions ? ExtractToolTypes<T['tools']> : Record<string, never>;
    Prompts: T['prompts'] extends PromptDefinitions ? ExtractPromptTypes<T['prompts']> : Record<string, never>;
};
