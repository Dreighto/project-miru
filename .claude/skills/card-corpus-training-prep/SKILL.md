---
name: card-corpus-training-prep
description: Govern the contract between card data and AI training input. Six clean-corpus criteria (no duplicates, full provenance, verified evidence source, real confidence, no sentinel dates, verified state), evidence weight tiers, dedup discipline, corpus rerun snapshot+hash protocol. Use when prepping data for Hermes / knowledge-layer training, deciding if rows are training-ready, scoping a corpus rerun, or designing fact pipelines. Triggers include train Hermes, prep the corpus, is this training-ready, knowledge layer, AI training on cards, fact extraction, OP01 clean corpus, corpus rerun, evidence weight, provenance.
---
