#!/usr/bin/env python3
"""
Populate EB04 rows in card_catalog.db from the official EN cardlist HTML.

EB04 modals live under series-filtered cardlist pages (root /cardlist/ does not list them).
This script fetches two Bandai series IDs (569115 OP15-EB04, 569114 OP14-EB04), concatenates
the HTML, and parses modals for ids matching EB04-NNN (base prints only, not parallel _pN).

Usage (from repo root):
  python tools/miru_ingest_eb04_official_cardlist.py [--dry-run] [--commit]
...
# OP15-EB04 and OP14-EB04 series pages (both required; canonical codes are EB04-001 … EB04-061)
SERIES_URLS = (
    "https://en.onepiece-cardgame.com/cardlist/?series=569115",
    "https://en.onepiece-cardgame.com/cardlist/?series=569114",
)
...
# Base print only: EB04-001, not EB04-001_p1
BASE_ID_RE = re.compile(r"^EB04-\d{3}$")


def _fetch_eb04_html() -> str:
    parts: list[str] = []
    for i, url in enumerate(SERIES_URLS):
        ...
        with urllib.request.urlopen(req, timeout=120) as resp:
            parts.append(resp.read().decode("utf-8", "replace"))
    return "\n".join(parts)
...
def _parse_modal_block(block: str) -> dict[str, str]:
    """Parse one <dl class=\"modalCol\" id=\"EB04-NNN\">...</dl> inner HTML."""
...
def parse_eb04_modals(html: str) -> dict[str, dict[str, str]]:
    """Return canonical_code -> field dict for base EB04-NNN modals."""
    out: dict[str, dict[str, str]] = {}
    for m in re.finditer(
        r'<dl class="modalCol" id="(EB04-\d{3})">(.*?)</dl>\s*',
        html,
        re.DOTALL,
    ):
        cid = m.group(1)
...
def _verify_eb04_empty_cardtypes(conn: sqlite3.Connection) -> list[tuple]:
    cur = conn.execute(
        """
        SELECT canonical_code, card_name, card_type, color
        FROM cards
        WHERE set_code = 'EB04'
...
def run(*, dry_run: bool) -> tuple[dict[str, int | str], str]:
    html = _fetch_eb04_html()
    modals = parse_eb04_modals(html)
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Ingest EB04 cards from official EN cardlist HTML (two series pages)."
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
    ...
    stats, html = run(dry_run=dry_run)
    ...
    conn = sqlite3.connect(DB_PATH, timeout=60)
    try:
        empty_rows = _verify_eb04_empty_cardtypes(conn)
    finally:
        conn.close()
    print("verification: EB04 rows with empty card_type count:", len(empty_rows))
    for canonical_code, card_name, card_type, color in empty_rows:
        print(f"  {canonical_code!r} | name={card_name!r} | card_type={card_type!r} | color={color!r}")
