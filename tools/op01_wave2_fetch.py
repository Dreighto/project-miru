#!/usr/bin/env python3
"""
op01_wave2_fetch.py -- Controlled fetch pass for Wave 2 (non-PRB01)

Downloads images from official OPTCG API URLs and writes them to
the appropriate provenance folders under D:\\Miru_Assets. Writes a results CSV.

NO DB WRITES.  NO REGISTRATION.  NO PM CHANGES.  NO WAVE 1.  NO SKIP ROWS.
"""
...
OUTPUT_CSV = ROOT / "data" / "overlays" / "op01_wave2_fetch_results.csv"

USER_AGENT = "ProjectMiru/1.0 (OP01-wave2-fetch; controlled pass; 1s pacing)"
HTTP_TIMEOUT = 30
PACING_SEC = 1.0
...
def main() -> int:
    print("=" * 70)
    print("OP01 WAVE 2 CONTROLLED FETCH PASS")
    print("=" * 70)
    ...
    wave2 = [
        ...
        and r["acquisition_wave"] == "2"
    ]

    total = len(wave2)
    print(f"  Selected Wave 2 rows: {total}")
    if total != 16:
        print(f"  STOP: Expected 16, got {total}")
    ...
    for idx, r in enumerate(wave2, 1):
        ...
        if local_path.is_file() and local_path.stat().st_size > 0:
            ...
            print(f"  [{idx:>2}/{total}] SKIPPED_EXISTING  {local_path.name}  ({size} bytes)")
            results.append({
                "card_code": card_code,
        ...
        if data is None or len(data) == 0:
            ...
            print(f"  [{idx:>2}/{total}] FAILED            {local_path.name}  reason={reason}")
            results.append({
                "card_code": card_code,
                "provenance_folder": provenance,
                "derived_full_local_path": str(local_path),
        ...
        # Verify
        if local_path.is_file() and local_path.stat().st_size > 0:
            print(f"  [{idx:>2}/{total}] FETCHED           {local_path.name}  ({written} bytes)")
            results.append({
                "card_code": card_code,
                "provenance_folder": provenance,
                "derived_full_local_path": str(local_path),
    ...
    print("VERIFICATION")
    print("=" * 70)
    print(f"  Selected row count:      {total}")
    ...
    print(f"  PRB01-WAVE1 rows touched: no")
    print()
