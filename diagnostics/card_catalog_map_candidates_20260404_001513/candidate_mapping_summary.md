...
**Generated (UTC):** 20260404_001513
**Database:** `D:\dev\tcg-watcher-worktree\data\card_catalog.db` (read-only)

## Scope

- Variants with **no** `printing_market_map` row where `printing_id = card_variants.id`
- And **no** usable tcgplayer column price
- And **no** existing chain price via map (defensive; normally implied by missing map)
...
| Manual review (medium/low confidence and/or `needs_manual_review=1` with a candidate pk) | 1991 |
